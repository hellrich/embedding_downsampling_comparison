#!/usr/bin/perl -w
# Modified version of Peter Kolb's https://www.dropbox.com/s/p3ta9spzfviovk0/xml2txt.pl
...
#                      </h>).
#          -article-per-line	Output each article on a single line. 
...
for($i = 0; $i <= $#ARGV; $i++){
    ...
    }elsif( $ARGV[$i] eq "-article-per-line" ){
	$option_article_per_line = 1;
	print STDERR "outputting one article per line\n";
